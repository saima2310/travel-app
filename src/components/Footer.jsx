    ...
    <>
      ...
      <footer className='bg-gradient-to-b from-emerald-400 to-green-300 pt-24 pb-7'>
        ...
        <div className='grid items-center grid-cols-3 justify-items-center'>
          {titles?.map((val, i) => (
            <div key={i} className='grid items-center justify-items-center mb-4'>
              <h1 className='text-xl lg:text-base uppercase font-bold'>{val.title}</h1>
            </div>
          ))}
        ...
        <div className='w-7/12 lg:w-[95vw] m-auto mt-9'>
          ...
          <div className='flex items-center justify-between px-7 md:px-0 md:gap-5 md:flex-col-reverse'>
            <p className='text-sm md:text-center'>Copyright <sup className='text-base font-bold'>&copy;</sup> All Reserved Right 2023 <span className='font-semibold'>SSTACK DEVELOPERS</span> </p>
            {/* social media links */}
